  ...
  try {
    ...
    for (const d of docs) {
      ...
      map[d.key] = {
        ...
        label: d.label,
        rate: d.rate,
        additional_rate: d.additional_rate ?? fallback?.additional_rate ?? 0,
        ride: fallback?.ride ?? d.ride,
        vehicle_type: d.vehicle_type ?? fallback?.vehicle_type ?? 0,
        trip_type: d.trip_type ?? fallback?.trip_type ?? 0,
        buffer: d.buffer,
        window: d.window,
        capacity: d.capacity,
