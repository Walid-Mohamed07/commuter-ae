...
export default function TransactionsChart({
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/transactions/timeseries?${params}`, { cache: "no-store" })
  ...
  const hasVolume = data.series.some(
    (point) => point.sumEgp > 0 || point.count > 0,
  );
    ...
    >
      <AdminCard title="Collected revenue — last 30 days" padding="16px 20px">
        {hasVolume ? (
          ...
          <AdminLoadingState
            title="No collected transactions in this period"
            description="Chart will populate once payments are captured."
          />
        )}
      </AdminCard>
